...
def LidarLaserThread(frame, mapframe, robotPosition: Tuple[int, int], CenterPoint: Tuple[int, int], laserAngle: float) -> None:
...
def Lidar(frame, robot, mapframe) -> None:
    x, y = robot["position"]
    tx, ty = LidarCenterPoint(robot)
    for angle in range(-40, 41, 5):
        start_new_thread(LidarLaserThread, (frame, mapframe, robot["position"], (tx, ty), angle))
...
def ArrivalToGoal(mapframe, x: int, y: int, robot) -> bool:
    # Direction Decide
    goal_angle = radian2deregee(math.atan2(y - robot["position"][1], x - robot["position"][0]))
    angle = DiffAngle(robot["angle"], goal_angle)
    ...
    if dx != 0 or dy != 0:
        ...
        if angle <= -3:
            ...
            d_a = 0
            robot["angle"] = goal_angle
            assert robot["angle"] == goal_angle, f'{robot["angle"]} == {goal_angle}'
        robot["angle"] += d_a
    if angle == 0:
        robot["v"] = 3
    else:
    ...
    if abs(robot["position"][0] - x) < 3 and abs(robot["position"][1] - y) < 3:
        robot["v"] = 0
        return True
    else:
        return False